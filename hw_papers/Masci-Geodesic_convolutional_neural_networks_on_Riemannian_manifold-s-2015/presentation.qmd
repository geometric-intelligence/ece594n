...
Example:
![](images/480px-Circle_with_overlapping_manifold_charts.png){width=50%}

# Riemannian Metric

A Riemannian metric assigns to each point $p$ in our manifold, a positive definite inner product $g_p: T_pM \times T_pM \rightarrow \mathbb{R}$
...
1. $v_{\rho}(x,y) \propto e^{\frac{-(d_X(x,y) - \rho)^2}{\sigma^2_\rho}}$ 
2. $v_{\theta}(x,y) \propto  e^{\frac{-d_X(\Gamma(x,\theta),y)^2}{\sigma^2_\theta}}$
...
::: notes
We want our shape description to be invariant of pose and how the object is deformed. We can see that the model does well compared to other shape descriptors. 
:::
...
::: notes
Want to compare how little our colors change between poses.
:::
