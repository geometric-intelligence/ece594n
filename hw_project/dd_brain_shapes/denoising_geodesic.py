...
class DenoisingGeodesic(BaseEstimator):
    r"""Geodesic Regression.
    ...
    def _model(self, X, coef, intercept):
        ...
        '''
        for ind,c_ in enumerate(coef):
            if ind == 0:
                pred = self.metric.exp(X_copy * c_, intercept)
            else:
                pred = self.metric.exp(pred * c_, intercept)
        return pred
        '''
        return self.metric.exp(X_copy * coef[None], intercept)

    def denoised_sample(self, noisy_samples, denoisers):
        return (noisy_samples@denoisers[0]) + (np.random.standard_normal(noisy_samples.shape))

    def _loss(self, X, y, clean, param, shape, weights=None):
    ...
    def fit(self, X, y, clean, weights=None, compute_training_score=False):
        ...
        if self.method == "riemannian":
            return self._fit_riemannian(times, y, clean, weights, compute_training_score)
    ...
    def _fit_riemannian(self, X, y, clean, weights=None, compute_training_score=False):
        ...
        objective_with_grad = gs.autodiff.value_and_grad(
            lambda param: self._loss(X, y, clean, param, shape, weights)
        )
        ...
        #i_h,c_h = intercept_hat, coef_hat
        current_loss = [math.inf]
        current_grad = gs.zeros_like(param)
        current_iter = i = 0
        for i in range(self.max_iter):
            # get initial loss
            loss, grad = objective_with_grad(param)#[i_h,c_h])
            #logging.info(f'loss {loss}')# grad {grad[0].shape} {grad[1].shape}')
            if gs.any(gs.isnan(grad[0])) or gs.any(gs.isnan(grad[1])):
            ...
            param = gs.concatenate([gs.flatten(intercept_hat_new), gs.flatten(coef_hat_new)])
            #i_h,c_h = intercept_hat_new, coef_hat_new
