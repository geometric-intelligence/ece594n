...
def projection_to_two_d(projections, points):
    ...
    """Takes Grassmannian generated projection matrix and converts it to a plotable 2D point.
    
    INPUT: Array of 2x2 projection matricies or a single projection matrix
...
def projection_to_three_d(projections, points):
    ...
    if points==1:
        ...
        return vector
    for i in range(points):
        x,y,z = np.matmul(projections[i], vector)/np.linalg.norm(np.matmul(projections[i], vector))
        vector = [x,y,z]
        storage.append(vector)
    return np.asarray(storage)
...
class Grassmannian(LevelSet):
    ...
    def plot(self, ticks):
        """Plots a Grassmannian manifold using the projection matricies generated by the Grassmannian class.
        Works for cases with k=1 and n=2,3. Code will be generalized for future use.
        
        INPUT: Grassmannian manifold and True/False for ticks to determine if you want labels
        ...
        if k > 1 or n > 3:
            print("Cannot handle sizes of k larger than 1 at this time. Cannot handle n larger than 3 at this time.")
            return -1
        if n == 2:
            points = 10000
            ...
            if ticks==True:
                pi = np.pi
                thetas = [pi/2, pi/3, pi/4, pi/6, 0, 11*pi/6, 7*pi/4, 5*pi/3, 3*pi/2]
                for i in thetas:
        ...
        if n == 3:
            ...
            for i in range(points):
                vector = np.random.rand(3,1)
                x,y,z = np.matmul(bunch_of_projections[i], vector)/np.linalg.norm(np.matmul(bunch_of_projections[i], vector))
                #print("The coords are: ",x,y,z)
                ax.plot(x,y,abs(z),'sk',alpha=.3)
    ...
    def plot_rendering(self,ticks):
        ...
        """Plots a Grassmannian manifold using the projection matricies generated by the Grassmannian class but with randomly
           sampled data.
        ...
        if n == 3:
            ...
            for i in range(point_draw):
                vector = np.random.rand(3,1)
                x,y,z = np.matmul(bunch_two[i], vector)/np.linalg.norm(np.matmul(bunch_two[i], vector))
                #print("The coords are: ",x,y,z)
                ax.plot(x,y,abs(z),'sm', alpha=.3)
    ...
    def plot_tangent_space(self):
        ...
        """WIP DONT USE!!!!!! ONLY WORKS IN 2D BUT MIGHT NOT EVEN WORK RIGHT SO FOR FUTURE DIRECTIONS
